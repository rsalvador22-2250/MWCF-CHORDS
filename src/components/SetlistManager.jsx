...
function SetlistManager({ songs }) {
  ...
  // Save transposes to localStorage
  useEffect(() => {
  ...
  // Load setlist songs when selected setlist changes
  useEffect(() => {
    if (selectedSetlist) {
      loadSetlistSongs();
    }
  ...
  const moveSongDown = (idx) => { 
    if (idx === setlistSongs.length-1) return; 
    const newOrder = [...setlistSongs]; 
    [newOrder[idx], newOrder[idx+1]] = [newOrder[idx+1], newOrder[idx]]; 
    setSetlistSongs(newOrder); 
  };
  ...
  // If viewing a song, render the full song page
  if (viewingSong) {
    const amount = transposeAmounts[viewingSong.id] || 0;
    return (
      <div className="setlist-container" style={{ display: 'block' }}>
        ...
        <div style={{ background: 'var(--bg-primary)', borderRadius: 24, overflow: 'hidden', boxShadow: 'var(--shadow-lg)', border: '1px solid var(--border-light)' }}>
          <div style={{ background: 'var(--gradient-primary)', padding: '16px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
              <div>
                <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: 'white' }}>{viewingSong.title}</h2>
                <div style={{ marginTop: 8, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <span style={{ background: 'rgba(255,255,255,0.2)', padding: '2px 10px', borderRadius: 20, fontSize: 12 }}>🎵 Original: {viewingSong.key}</span>
                  {amount !== 0 && <span style={{ background: '#ffc107', color: '#333', padding: '2px 10px', borderRadius: 20, fontSize: 12 }}>➡️ Current: {getCurrentKey(viewingSong)}</span>}
                  {viewingSong.tempo && <span style={{ background: 'rgba(255,255,255,0.2)', padding: '2px 10px', borderRadius: 20, fontSize: 12 }}>🎚️ {viewingSong.tempo} BPM</span>}
                  {viewingSong.song_date && <span style={{ background: 'rgba(255,255,255,0.2)', padding: '2px 10px', borderRadius: 20, fontSize: 12 }}>📅 {new Date(viewingSong.song_date).toLocaleDateString()}</span>}
                </div>
              </div>
            </div>
          </div>
          <div style={{ padding: '12px 20px', background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border-light)', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 13, fontWeight: 600 }}>🎼 Transpose:</span>
            <button onClick={() => transposeSong(viewingSong.id, -1)} style={{ background: 'var(--primary-600)', color: 'white', border: 'none', padding: '4px 12px', borderRadius: 20, cursor: 'pointer', fontSize: 12 }}>-1</button>
            <button onClick={() => transposeSong(viewingSong.id, 1)} style={{ background: 'var(--primary-600)', color: 'white', border: 'none', padding: '4px 12px', borderRadius: 20, cursor: 'pointer', fontSize: 12 }}>+1</button>
            {amount !== 0 && <button onClick={() => resetTranspose(viewingSong.id)} style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-light)', padding: '4px 12px', borderRadius: 20, cursor: 'pointer', fontSize: 12 }}>Reset</button>}
            <span style={{ background: 'var(--bg-primary)', padding: '4px 12px', borderRadius: 20, fontSize: 12 }}>Current Key: {getCurrentKey(viewingSong)}</span>
          </div>
          <div style={{ padding: 24 }}>
            <div style={{ background: 'var(--bg-secondary)', padding: 24, borderRadius: 20, maxHeight: '60vh', overflowY: 'auto' }}>
              {formatLyricsFull(viewingSong)}
            </div>
            {viewingSong.notes && (
              <div style={{ marginTop: 20, padding: 16, background: '#fef3c7', borderRadius: 16, borderLeft: '4px solid #f59e0b' }}>
                <strong style={{ color: '#92400e' }}>📝 Notes:</strong>
                <p style={{ marginTop: 8, color: '#78350f' }}>{viewingSong.notes}</p>
              </div>
            )}
  ...
  return (
    <div className="setlist-container">
      ...
      <div className="setlist-content">
        {selectedSetlist ? (
          <>
            <div style={{ marginBottom: 16, paddingBottom: 10, borderBottom: '1px solid var(--border-light)' }}>
              <h2 style={{ margin: 0, fontSize: 18 }}>📋 {selectedSetlist.name}</h2>
              {selectedSetlist.service_date && <p style={{ marginTop: 4, fontSize: 11, color: 'var(--text-tertiary)' }}>🎯 {formatDate(selectedSetlist.service_date)}</p>}
            </div>
            <div style={{ marginBottom: 20 }}>
              <h3 style={{ fontSize: 14, marginBottom: 10 }}>🎵 Songs ({setlistSongs.length})</h3>
            ...
            <div style={{ marginTop: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, flexWrap: 'wrap', gap: 8 }}>
                <h3 style={{ fontSize: 14 }}>➕ Add Songs</h3>
                <div>
                  <button onClick={selectAll} style={{ background: 'var(--bg-tertiary)', padding: '4px 10px', borderRadius: 6, fontSize: 11, marginRight: 8 }}>Select All</button>
                  <button onClick={addMultipleToSetlist} disabled={selectedSongs.length===0} style={{ background: selectedSongs.length===0 ? 'var(--border-medium)' : 'var(--gradient-success)', color: 'white', padding: '4px 12px', borderRadius: 6, fontSize: 11 }}>Add ({selectedSongs.length})</button>
                </div>
              </div>
              <input type="text" placeholder="🔍 Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--border-light)', borderRadius: 8, marginBottom: 10, fontSize: 12 }} />
              <div className="songs-grid">
                {filteredSongs.map(song => (
                  <div key={song.id} className="available-song-card" onClick={() => toggleSelectSong(song)} style={{ cursor: 'pointer', background: selectedSongs.find(s => s.id === song.id) ? 'var(--primary-100)' : 'var(--bg-secondary)' }}>
                    <div>
                      <input type="checkbox" checked={selectedSongs.find(s => s.id === song.id)} onChange={()=>{}} onClick={e=>e.stopPropagation()} style={{ marginRight: 6 }} />
                      <strong>{song.title}</strong>
                      <div><small>Key: {song.key}</small></div>
                    </div>
                    <button onClick={(e)=>{e.stopPropagation(); addToSetlist(song);}} style={{ background: 'var(--gradient-success)', color: 'white', border: 'none', padding: '4px 10px', borderRadius: 6, fontSize: 11 }}>+ Add</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ textAlign: 'center', padding: 40 }}>
            ...
            <button className="btn-primary" onClick={() => setShowCreate(true)} style={{ width: 'auto', marginTop: 12 }}>+ Create New</button>
          </div>
        )}
      </div>
      {showCreate && (
        <div className="modal" onClick={() => setShowCreate(false)}>
          <div className="modal-content" onClick={e=>e.stopPropagation()}>
            ...
            <input type="text" placeholder="Name" value={newSetlistName} onChange={e=>setNewSetlistName(e.target.value)} autoFocus />
